...
export function useFetch(url, method = "GET", postProcessingFunctionName = 'json') {
  ...
  useEffect(() => {
    if (typeof url !== 'string')
      throw new Error("useFetch expects a string as an URL");
    const urlWithBackend = url.startsWith('/') ? BACKEND_URL + url : url;
    console.log('url :' + urlWithBackend)
    setData(null);
    setError(null);
    setIsLoading(false);
    if (!url)
